...
const patientLogin = async (userdata, role ,res) => {
    ...
    if(!user){
        return res.status(404).json({
            message: "No Patient registered by this username",
            success: false
        });
    }
    if(user.role !== role){
        return res.status(403).json({
            message: "Unauthorized this webpage can only be accessed by a Patient",
            success: false
        });
    }
    ...
    if(isMatch){
        ...
        return res.status(200).json({
            otoken,
            message: "Patient Logging Successful",
            success: true
        });
...
const clerkLogin = async (userdata, role ,res) => {
    let {username, password} = userdata;
    ...
    if(!user){
        return res.status(404).json({
            message: "No Clerk registered by this username",
            success: false
        });
    }
    if(user.role !== role){
        return res.status(403).json({
            message: "Unauthorized this webpage can only be accessed by a Clerk",
            success: false
        });
    }
    ...
    if(isMatch){
        ...
        return res.status(200).json({
            otoken,
            message: "Clerk Logging Successful",
            success: true
        });
...
const doctorLogin = async (userdata, role ,res) => {
    let {username, password} = userdata;
    ...
    if(!user){
        return res.status(404).json({
            message: "No Doctor registered by this username",
            success: false
        });
    }
    if(user.role !== role){
        return res.status(403).json({
            message: "Unauthorized this webpage can only be accessed by a Doctor",
            success: false
        });
    }
    ...
    if(isMatch){
        ...
        return res.status(200).json({
            otoken,
            message: "Doctor Logging Successful",
            success: true
        });
